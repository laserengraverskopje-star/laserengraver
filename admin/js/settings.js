...
async function load(){
  try{const r=await fetch('/api/settings',{cache:'no-store',headers:{'Authorization':`Bearer ${tok()}`}});const d=await r.json();if(r.status===401){location.href='login.html';return;}if(!r.ok||!d.success&&!d.siteName)throw new Error(d.error||'Грешка при читање поставки.');
...
async function save(){
 const settings={siteName:siteName.value.trim(),siteTitle:siteTitle.value.trim(),siteDescription:siteDescription.value.trim(),phone:phone.value.trim(),email:email.value.trim(),address:address.value.trim(),mapsUrl:mapsUrl.value.trim(),facebookUrl:facebookUrl.value.trim(),instagramUrl:instagramUrl.value.trim(),workingHours:workingHours.value.trim(),notificationEmail:notificationEmail.value.trim(),notifyMessages:String(notifyMessages.checked),notifyOffers:String(notifyOffers.checked),offersEnabled:String(offersEnabled.checked),offerRequirePhone:String(offerRequirePhone.checked),offerRequireService:String(offerRequireService.checked),offerSuccessMessage:offerSuccessMessage.value.trim(),maxExtraImages:maxExtraImages.value,galleryColumns:galleryColumns.value,openMainOnExtraClick:String(openMainOnExtraClick.checked)};
